...
let newNumbers = numbers.filter((x)=>x% 2 !== 0).map((y)=>y*2);
console.log();
...
//1980s movies Count
let movies80s = movies.filter((y)=>(y.year>=1980 && y.year<1990));
console.log();
console.log(movies80s);
console.log();
console.log("There are " + movies80s.length + " movies from 1980 to 1989");

// creating a new array with new key(tag).
 
let tagKey=(x)=>{ 
            if(x.rating >= 7) {
                x.tag ="Good" };
            if(x.rating >= 4 && x.rating < 7) {
                x.tag ="Average" }
            if(x.rating < 4) {
                x.tag ="Bad" } 
            return x;
...
let movieWithTag = movies.map(tagKey);
console.log();
console.log(movieWithTag);

//Chaining
const hiRatingMovies = (x)=>x.rating > 6;
const ratings = (y)=> {return temp = y.rating}

let ratingArray = movies.filter(hiRatingMovies)
                        .map(ratings);
console.log();
console.log(ratingArray);

// calculating the average rating using reduce
let sumOfRating = movies.reduce((sum,x)=> {return sum + x.rating},0);
console.log();
console.log("The average rating is " + (sumOfRating/movies.length).toFixed(2));
